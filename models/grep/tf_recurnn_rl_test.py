...
if __name__ == '__main__':
    ...
    # read good params
    params1 = pickle.load(open('./good05_params_bn1.pkl'))
    W_n1, b_n1, W_n2, b_n2 = params1
    e_param = [W_n1, b_n1, W_n2, b_n2]
    # e_param = [tW_n1, tb_n1, tW_n2, tb_n2]

    params2 = pickle.load(open('./good01_params_rl4_2.pkl'))
    # params2 = pickle.load(open('./params_rl4_2.pkl'))
    s_param = params2[1]
    ...
    for idx in range(len(tx)):
        ...
        sum_r += r0
        # mis = mises[idx]
        print('index', idx, 'reward', r0, cs[idx],  nexpr[8:10], ms[:3])
        # if all(mis == ms[:len(mis)]):
        #     mcorrect += 1
        if np.argmax(nexpr[:10]) == ty[idx]:
            correct += 1
            tcorrect += 1
            # if nexpr[ty[idx]] > 0.8:
            # else:
